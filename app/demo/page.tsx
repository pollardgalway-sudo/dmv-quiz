...
export default function DemoPage() {
    ...
    useEffect(() => {
        ...
        fetch('/data/questions-basics.json')
            ...
            .then(data => {
                // 只取前20道题
                setQuestions(data.slice(0, DEMO_LIMIT))
            })
    }, [])
    ...
    return (
        <div className="min-h-screen page-gradient transition-colors duration-500">
            ...
            <div className="fixed inset-0 overflow-hidden pointer-events-none">
                <div className="absolute -top-40 -right-40 w-80 h-80 bg-gradient-to-br from-orange-400/20 to-amber-400/20 rounded-full blur-3xl animate-float" />
            ...
            <nav className="relative z-10 glass border-b border-white/20 dark:border-white/5 sticky top-0">
                <div className="container mx-auto px-4 py-3 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        {/* Demo Badge */}
                        <span className="px-3 py-1 text-xs font-bold rounded-full bg-gradient-to-r from-orange-500 to-amber-500 text-white">
                            🎁 免费试用
                        </span>
                    </div>
            ...
            <div className="relative z-10 container mx-auto px-4 py-6 max-w-4xl">
                ...
                {/* Demo Notice Banner */}
                {!isCompleted && (
                    <div className="mb-4 p-3 rounded-xl bg-gradient-to-r from-orange-500/10 to-amber-500/10 border border-orange-500/30 text-center animate-fade-in-up">
                        <p className="text-sm">
                            🎁 <span className="font-medium">免费试用版</span> - 共 {DEMO_LIMIT} 道题目 ·
                            <span className="text-muted-foreground ml-1">完整版包含 600+ 道精选考题</span>
                        </p>
                    </div>
                )}
                ...
                {isCompleted ? (
                    <Card className="premium-card overflow-hidden animate-scale-in">
                        ...
                        <CardContent className="p-6 sm:p-8 text-center">
                            ...
                            <h2 className="text-2xl sm:text-3xl font-bold mb-2">试用完成！</h2>
                            <p className="text-lg text-muted-foreground mb-6">
                                您已完成 {DEMO_LIMIT} 道免费试用题目
                            </p>
                            ...
                            <div className="flex flex-col gap-3">
                                {/* 小红书购买引导 */}
                                <a
                                    href="https://www.xiaohongshu.com/goods-detail/692d792a01312b0001a2730a"
                                    target="_blank"
                                    rel="noopener noreferrer"
                                >
                                    <Button
                                        className="w-full h-14 text-lg font-bold"
                                        style={{ background: 'linear-gradient(135deg, #FF2442, #FF6B6B)' }}
                                    >
                                        📕 返回小红书拍下继续使用
                                    </Button>
                                </a>
                                <p className="text-sm text-muted-foreground text-center">
                                    购买后获取激活码，解锁全部 600+ 道考题
                                </p>
                                <Link href="/">
                                    <Button
                                        variant="outline"
                                        className="w-full h-12 text-base font-medium glass border-2"
                                    >
                                        🔐 已有激活码？点击输入
                                    </Button>
                                </Link>
                    ...
                    <Card className="premium-card overflow-hidden animate-fade-in-up" style={{ animationDelay: '100ms' }}>
                        ...
                        <CardHeader className="p-4 sm:p-6">
                            <div className="flex items-start gap-3">
                                <span className="flex-shrink-0 w-8 h-8 rounded-full bg-gradient-to-br from-orange-400 to-amber-500 flex items-center justify-center text-white font-bold text-sm">
                                    {currentIndex + 1}
                                </span>
                        ...
                        <CardContent className="space-y-4 sm:space-y-6 p-4 sm:p-6 pt-0">
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 pt-4">
                                ...
                                >
                                    {currentIndex === questions.length - 1 ? '完成试用 →' : '下一题 →'}
                                </Button>
                            </div>
                        </CardContent>
                    </Card>
